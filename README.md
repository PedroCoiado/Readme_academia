...
* **Nome e natureza da organização:** Academia de Artes Marciais (Gerida pelo CEO Adalberto). É uma organização com fins lucrativos voltada para a prestação de serviços esportivos e ensino de lutas.
...
* **RN01 (Vencimento Fixo):** Toda mensalidade possui como data padrão de vencimento o quinto dia útil de cada mês.
* **RN02 (Limite de Alunos):** Uma turma não pode ultrapassar o limite máximo de 30 (trinta) alunos matriculados.
* **RN03 (Exclusividade do Professor):** Um professor só pode ser associado e ministrar aulas na modalidade em que possui propriedade/especialização.
* **RN04 (Trancamento por Lesão):** O aluno pode alterar seu status para "Trancado" caso sofra uma lesão, interrompendo temporariamente a cobrança ou contagem de tempo.
...
|-|-|-|
|id\_aluno (PK)|Código gerado pelo sistema para identificar o aluno|Chave Primária. Obrigatório e único.|
|cpf|Cadastro de Pessoa Física do aluno|Obrigatório e não pode ser duplicado.|
|nome\_completo|Nome completo do aluno|Obrigatório.|
|data\_nascimento|Data de nascimento do aluno|Obrigatório. Usada para cálculo de idade.|
|endereco|Endereço residencial do aluno|Opcional.|
|email|E-mail de contato do aluno|Opcional.|
|telefone|Telefone principal de contato|Obrigatório.|
|telefone\_reserva|Telefone alternativo/de emergência|Opcional.|
|problema\_saude|Registro de condições de saúde relevantes (Ex: tipo sanguíneo, lesões, restrições)|Dado sensível (RNF02). Informação para emergências.|
|esportes\_praticados|Modalidades que o aluno já praticou ou pratica|Alimenta o histórico de treinos anteriores (RF01).|
|situacao|Status do aluno no sistema|Deve ser: 'Ativo', 'Inativo' ou 'Trancado' (RN04).|

### Entidade: PROFESSOR

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_professor (PK)|Código gerado pelo sistema para identificar o professor|Chave Primária. Obrigatório e único.|
|cpf|Cadastro de Pessoa Física do professor|Obrigatório e não pode ser duplicado.|
|nome\_completo|Nome completo do professor/instrutor|Obrigatório.|
|data\_nascimento|Data de nascimento do professor|Obrigatório.|
|endereco|Endereço residencial do professor|Opcional.|
|email|E-mail de contato do professor|Opcional.|
|telefone|Telefone de contato do professor|Obrigatório.|
|problema\_saude|Registro de condições de saúde relevantes|Dado sensível (RNF02).|
|especializacao|Modalidade(s) em que o professor é especializado|Só pode lecionar na modalidade em que tem propriedade/especialização (RN03).|
|nivel\_experiencia|Nível de experiência/graduação do professor na modalidade|Usado para validar a aptidão a lecionar.|
|registro\_qualificacoes|Certificações, faixas e qualificações do professor|Comprovação da especialização (RN03).|

### Entidade: MODALIDADE

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_modalidade (PK)|Código gerado pelo sistema para identificar a modalidade|Chave Primária. Obrigatório e único.|
|nome\_modalidade|Nome da modalidade (Muay Thai, Kickboxing, Jiu-Jitsu, Boxe, Capoeira, No-Gi)|Obrigatório. Limitado às 6 modalidades oferecidas.|
|data\_inicio\_aulas|Data em que a modalidade passou a ser oferecida|Obrigatório.|
|possui\_sistema\_faixas|Indica se a modalidade usa sistema de faixas/graduação|Define se a entidade GRADUAÇÃO se aplica (algumas modalidades, como Boxe, usam análise técnica em vez de faixa).|
|status|Situação atual da modalidade (ativa/inativa)|Controla se novas turmas podem ser abertas.|
|motivo\_trancamento|Motivo de eventual suspensão da modalidade|Preenchido apenas quando status = inativa.|
|vagas\_ocupadas|Total de vagas já ocupadas somando as turmas da modalidade|Apoia o controle do limite de 30 alunos por turma (RN02).|

### Entidade: TURMA

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_turma (PK)|Código gerado pelo sistema para identificar a turma|Chave Primária. Obrigatório e único.|
|dia\_semana|Dia(s) da semana em que a turma acontece|Obrigatório.|
|horario\_inicio|Horário de início da aula|Obrigatório.|
|horario\_fim|Horário de término da aula|Obrigatório.|

*Observação: a turma está vinculada a uma MODALIDADE, a um PROFESSOR e, por meio de MATRÍCULA, aos alunos participantes, respeitando o limite de 30 alunos (RN02).*

### Entidade: MATRÍCULA

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_matricula (PK)|Código gerado pelo sistema para identificar a matrícula|Chave Primária. Obrigatório e único.|
|data\_matricula|Data em que o aluno se matriculou na turma|Obrigatório. Substitui a ficha física atual, que não gerava número de matrícula.|
|status|Situação da matrícula (Ex: ativa, cancelada, trancada)|Reflete o status do aluno (RN04) na turma específica.|

### Entidade: PAGAMENTO

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_pagamento (PK)|Código identificador do recebimento|Chave Primária. Obrigatório.|
|data\_pagamento|Data em que o aluno realizou o pagamento|Obrigatório.|
|plano\_contratado|Tipo de plano que está sendo pago|Deve ser: 'Mensal', 'Trimestral', 'Anual' ou 'Família'.|
|status\_pagamento|Situação do pagamento (Ex: pago, atrasado, pendente)|Resolve o problema de inadimplência não registrada. Vencimento padrão no 5º dia útil (RN01).|

### Entidade: PRESENÇA

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_presenca (PK)|Código gerado pelo sistema para identificar o registro de presença|Chave Primária. Obrigatório e único.|
|data\_aula|Data em que a aula ocorreu|Obrigatório (RF05).|
|horario\_aula|Horário em que a aula ocorreu|Obrigatório.|
|presenca|Indica se o aluno esteve presente ou ausente|Obrigatório. Base para a elegibilidade a exames de faixa (RF06).|

### Entidade: GRADUAÇÃO

|Atributo|Descrição|Regra de negócio associada|
|-|-|-|
|id\_graduacao (PK)|Código gerado pelo sistema para identificar o registro de graduação|Chave Primária. Obrigatório e único.|
|nivel\_faixa|Faixa/nível obtido pelo aluno na graduação|Obrigatório nas modalidades com sistema de faixas (RF06).|
|data\_graduacao|Data em que o exame/graduação ocorreu|Obrigatório.|
|observacao|Observações do professor sobre a avaliação (Ex: análise técnica no Boxe)|Complementa a avaliação quando não há sistema de faixas.|
